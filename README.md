Работа выполена конч*** долба*** из группы П.22.3-1 
...
#58  базовый класс Logger с виртуальной функцией log(), которая выбрасывает исключение при ошибке записи в лог. классы FileLogger и ConsoleLogger, которые имеют разные способы логирования.

  
#59  класс Notification с виртуальной функцией send(), которая выбрасывает исключение при ошибке отправки уведомления.классы EmailNotification, SMSNotification и PushNotification, которые имеют свои способы отправки уведомлений.
  
#60  класс Shape с виртуальной функцией getPerimeter(), которая выбрасывает исключение при некорректных параметрах.классы Circle, Rectangle и Triangle, которые вычисляют свои периметры.
  
#61  
Session с виртуальной функцией start(), которая выбрасывает исключение при попытке начать уже активную сессию.классы User Session и AdminSession, которые имеют разные правила управления сессиями.
  
#62  
базовый класс DataProcessor с виртуальной функцией process(), которая выбрасывает исключение при некорректных данных. классы CSVProcessor, JSONProcessor и XMLProcessor, которые обрабатывают разные форматы данных.


#63
класс Shape с виртуальной функцией transform(), которая выбрасывает исключение при некорректных параметрах трансформации.классы Circle, Square и Polygon, которые имеют свои правила трансформации.
  
#64
класс User  с виртуальной функцией updateProfile(), которая выбрасывает исключение при некорректных данных профиля.классы AdminUser  и RegularUser , которые имеют разные поля профиля.
  
#65
класс TaskManager с виртуальной функцией runTask(), которая выбрасывает исключение при ошибке выполнения задачи.классы ScheduledTask и ImmediateTask, которые имеют разные способы выполнения задач.
  
#66 
базовый класс Shape с виртуальной функцией getColor(), которая выбрасывает исключение при некорректных параметрах.классы Circle, Square и Triangle, которые возвращают свои цвета.
  
#67 
класс PaymentGateway с виртуальной функцией processPayment(), которая выбрасывает исключение при ошибке обработки платежа.классы StripeGateway, PayPalGateway и SquareGateway, которые имеют свои способы обработки платежей.
  
 #68 
 класс FileReader с виртуальной функцией read(), которая выбрасывает исключение при ошибке чтения файла. классы TextFileReader, BinaryFileReader и JsonFileReader, которые обрабатывают разные форматы файлов.
  
#69 
класс Shape с виртуальной функцией getArea(), которая выбрасывает исключение при некорректных параметрах.классы Circle, Rectangle и Triangle, которые вычисляют свои площади с учетом возможных ошибок.
  
#70 
базовый класс ExceptionHandler с виртуальной функцией handle(), которая выбрасывает исключение при некорректных данных.классы FileExceptionHandler, NetworkExceptionHandler и InputExceptionHandler, которые обрабатывают разные типы исключений.
  
 #71 
 класс User Profile с виртуальной функцией save(), которая выбрасывает исключение при ошибке сохранения профиля. классы AdminProfile и RegularProfile, которые имеют разные правила сохранения данных.
  
 #72 
 класс Shape с виртуальной функцией getDescription(), которая выбрасывает исключение при некорректных параметрах.классы Circle, Square и Triangle, которые возвращают свои описания.
  
#73  класс EmailService с виртуальной функцией sendEmail(), которая выбрасывает исключение при ошибке отправки. классы SMTPService, APIService и MockEmailService, которые имеют разные способы отправки электронной почты.
  
 #74 базовый класс DataValidator с виртуальной функцией validate(), которая выбрасывает исключение при некорректных данных. классы EmailValidator, PhoneValidator и AgeValidator, которые проверяют разные типы данных.
  
#75  класс Shape с виртуальной функцией getDimensions(), которая выбрасывает исключение при некорректных параметрах. классы Circle, Rectangle и Triangle, которые возвращают свои размеры.

  
 #76 класс Transaction с виртуальной функцией execute(), которая выбрасывает исключение при недостаточном балансе или некорректных данных. классы DepositTransaction и WithdrawalTransaction, которые имеют свои правила выполнения транзакций.
  
 #77 класс Shape с виртуальной функцией getIntersection(), которая выбрасывает исключение при попытке найти пересечение между фигурами с некорректными параметрами.классы Circle, Rectangle и Polygon, которые имеют свои правила нахождения пересечений.
  
#78  класс User  с виртуальной функцией authenticate(), которая выбрасывает исключение при неверных учетных данных.  классы AdminUser  и RegularUser , которые имеют разные методы аутентификации.
  
#79
Скласс DataProcessor с виртуальной функцией processData(), которая выбрасывает исключение при некорректных данных. классы CSVDataProcessor, XMLDataProcessor и JSONDataProcessor, которые обрабатывают разные форматы данных.
